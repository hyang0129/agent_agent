...
| `TestOutput` | Coding composite — Tester (test plan + execution results) | Test suite results, pass/fail status, assertion counts |
...
## [P12 — Worker Management](12-worker-management.md)

**The worker (`agent-agent worker`) is a developer tool — a standalone CLI process run manually in local or dev-container environments. It is entirely independent of the DAG server (`server.py`); they share Postgres state but have separate lifecycles. Nothing in this policy governs the server.** Workers are long-running processes that autonomously claim GitHub issues from a human-curated queue, execute DAG runs, and report results. Each worker generates a `worker_id` via `uuid4()` at startup (never reused). Workers write a registration record before polling begins; field additions require a policy amendment. Staleness is defined solely as `now() - last_heartbeat_at > 5 × WORKER_HEARTBEAT_INTERVAL_SECONDS` and is computed at read time — never written. The `work_claims` state machine has exactly four written statuses: `running`, `completed`, `failed`, `abandoned`; all terminal states are irreversible; reclaims use conditional UPDATE (never INSERT). The holding worker process is the sole writer to `work_claims.status`. `completed` requires a confirmed PR URL in `dag_runs`. Reclaiming a stale claim requires a resumption check before starting a new DAG run (P6.5a). Abandonment (label removed, issue closed, repo unreachable) stops execution at the next node boundary. Five eligibility checks are required before a claim attempt (label state, no active claim, no open agent PR, issue open, branch writable); failures emit skip events — not escalations. All checks must be re-run after a successful claim; any failure produces `abandoned` with no DAG run and no escalation comment. Budget is read from `WORKER_RUN_BUDGET_USD` only; unset causes worker exit. Per-repo concurrency cap (default 2) is enforced atomically in the claim CTE. Worker events share the `emit_event` path and `node_events` table with DAG events (Level 2, 90-day retention). Eleven mandatory worker events are defined; poll-cycle events are prohibited; `skip_reason` must be a `SkipReason` enum value with no catch-all. Three bookkeeping endpoints are required including queue age as the primary worker health signal.

**Violations include:** configuring or reusing `worker_id` across restarts; polling before the registration record is written; adding registration fields without a policy amendment; writing `stale` to `work_claims.status`; using any status other than the four defined; inserting a new row on reclaim instead of conditional UPDATE; non-worker processes writing to `work_claims.status`; marking `completed` before PR URL is confirmed; starting a fresh DAG run after reclaim without attempting resumption; starting a new DAG node after an abandonment trigger; skipping any eligibility check; using dry-run push for branch writability; computing or hardcoding budget; exceeding per-repo concurrency cap non-atomically; emitting worker events via a separate logging path; omitting `worker_id` from any worker event; missing any of the 11 mandatory events; emitting poll-cycle events; using catch-all enum values for `skip_reason`; storing worker events in a separate table.
